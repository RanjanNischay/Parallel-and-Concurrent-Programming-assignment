...
class TTASLock:public Lock{
  ...
  void lock(){
    while(true){
        ...
        if(!state.test_and_set(std::memory_order_acquire)) return;
    }
  }
  void unlock(){
      state.clear();
...
void testCS(int i, int n, int k, int t1, int t2,TTASLock* test){
    int id = i;
...
int main(){
    ...
    TTASLock* ttas;
    ttas = new TTASLock();
    ofstream fout("TTAS_output.txt");
    
    for(int i = 0; i<n;i++){
        
        t[i] = std::thread(testCS,i,n,k,t1,t2,ttas);
