...
void lock(){
 Qnode* mypred = tail.exchange(mynode);
 if(mypred != NULL){
  mynode->locked.store(true);
  mypred->next= mynode;
  //wait until predecessor gives up the lock
  while(mynode->locked.load()){ };  // busy waiting
 }
}
void unlock(){
    if(mynode->next == NULL){
...
void testCS(int i, int n, int k, int t1, int t2,MCSLock* test){
    int id = i;
    ...
    int time1 = (int)t1d;
    int time2 = (int)t2d;
    ofstream fout("MCS_output.txt", ios::app);
...
int main(){
    ...
    MCSLock* mcs;
    mcs = new MCSLock();
    ofstream fout("MCS_output.txt");
    
    for(int i = 0; i<n;i++){
        
        t[i] = std::thread(testCS,i,n,k,t1,t2,mcs);
